...
static class Program
{
    ...
    private static void MixPostsIntoOneFile()
    {
        var posts = new List<(string url, string message)>();
        foreach (var file in Directory.EnumerateFiles("comments", "comment*.json"))
        {
            ...
            posts.Add((url, message));
        }

        var outJson = posts.Select(p=>new { p.url, p.message }).ToList();
        var outJsonString = JsonSerializer.Serialize(outJson, new JsonSerializerOptions { WriteIndented = true });
        File.WriteAllText("comments/all_posts.json", outJsonString);
    }
